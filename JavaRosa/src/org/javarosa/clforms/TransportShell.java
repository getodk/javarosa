...
public class TransportShell extends MIDlet implements ApplicationInitializer,
	...
	private void configureLogger() {
		...
		/*
		 * try { log = new MIDPLogger(MIDPLogger.DEBUG,true,false); } catch
		 * (Exception e) { System.out.println("Exception creating MIDPLogger");
		 * e.printStackTrace(); }
		 */
	}
	...
	public void getNewFormsByTransportPropertySetting() {
		String method = PropertyManager.instance().getSingularProperty(
				"GetFormsMethod");
		...
		if (TextUtil.equalsIgnoreCase(method, Constants.GETFORMS_AUTOHTTP)) {
			callEvGetMeAUTO();
		} else if (TextUtil.equalsIgnoreCase(method,
				Constants.GETFORMS_BLUETOOTH)) {
			this.xformClient = new VisualXFormClient(this);
	...
	public void commandAction(Command c, Displayable d) {
		...
		// #if app.usefileconnections
			...
			String selectedFile = directory.getString(directory
					.getSelectedIndex());
	...
	private void loadProperties() {
		...
		Enumeration en = PropertyManager.instance().getRules()
				.allowableProperties().elements();
	...
	private void openFileIntoNewForm(String fileName) {
		try {
			...
			System.out.println("DIRECTORY: "
					+ this.fileBrowser.getSelectedFileURL());
			FileConnection fc = (FileConnection) Connector
					.open(this.fileBrowser.getSelectedFileURL());
			System.out.println("FILE SIZE: " + fc.fileSize());
			InputStream fis = fc.openInputStream();
			// So this used to use Double.toInt(), which is a problem because
			// most of these devices
			// don't have floating point processors, and the libraries don't
			// have the datatype.
			// The Double.toInt() method just cast the long to an integer
			// anyway, so this should do
			// the same thing
			...
			try {
				...
				System.out.println("writing form to: "
						+ this.xformRMS.getNextRecordID()
						+ form.getXmlModel().toString());
				System.out.println("Form metaD: "
						+ new XFormMetaData(form).toString());
				// TODO once write external fixed for Form change this to right
				// form object
				this.xformRMS.writeToRMS(form);
				Display.getDisplay(this).setCurrent(
						new javax.microedition.lcdui.Alert("save succes",
								"Form Loaded successfully", null,
								AlertType.CONFIRMATION), this.getFormList());
				int formNumber = this.xformRMS.getNextRecordID() - 1;
				System.out.println("written to:" + formNumber
						+ "Number of records : "
						+ this.xformRMS.getNumberOfRecords());
				// if immediately load form
				this.controllerLoadForm(formNumber);

			} catch (Exception e) {
				Display.getDisplay(this).setCurrent(
						new javax.microedition.lcdui.Alert("save error",
								"Form failed to Load", null, AlertType.ERROR),
						this.getFormList());
			}

			// TextBox viewer = new TextBox("XForm Contents", null, length,
