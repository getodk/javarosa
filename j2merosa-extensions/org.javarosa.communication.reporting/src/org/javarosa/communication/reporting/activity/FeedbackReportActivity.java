...
public class FeedbackReportActivity implements IActivity, CommandListener {
	...
	public void commandAction(Command c, Displayable d) {
		if(c.equals(FeedbackReportScreen.SEND_REPORT)) {
			...
			String url = DataCaptureServiceRegistry.instance().getPropertyManager().getSingularProperty(FeedbackReportProperties.FEEDBACK_REPORT_SERVER);
			
			String id = DataCaptureServiceRegistry.instance().getPropertyManager().getSingularProperty("DeviceID");
			
			//TODO: For now, only http is supported, so we hack this to switch to that.
			int httpmethod = (new HttpTransportMethod()).getId();
			DataCaptureServiceRegistry.instance().getTransportManager().setCurrentTransportMethod(httpmethod);
			...
			DataCaptureServiceRegistry.instance().getTransportManager().send(tmessage, httpmethod);
