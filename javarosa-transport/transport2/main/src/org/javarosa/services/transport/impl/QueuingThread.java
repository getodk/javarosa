...
public class QueuingThread extends Thread {
	...
	public final static int FETCHING = 2;
	/**
	 * 
	 */
	...
	/**
	 * TransportService.SEND or TransportService.FETCH
	 */
	 ...
	 *            queue
	 */
	public QueuingThread(int transportType, Transporter transporter,
	...
	public void run() {
		...
		while ((this.triesRemaining > 0) && !message.isSuccess()) {
			try {
				if (transportType == SENDING)
					message = attemptToSend();
				else
					message = attemptToFetch();
	 ...
	 * @return The message being sent (with updated status)
	 */
	private TransportMessage attemptToFetch() throws TransportException {
		System.out.println("Attempts left: " + this.triesRemaining);
		TransportMessage message = this.transporter.fetch();
		if (message.isSuccess()) {
			onSuccess(message);
		} else {
			onFailure(message);
	...
	private void onSuccess(TransportMessage message) throws TransportException {
		if (message.getTransportListener() != null) {
			try {
				message.getTransportListener().onSuccess();
			} catch (Exception e) {
				// badly defined listener shouldn't crash the thread
				// so catch all exceptions here and do nothing
				e.printStackTrace();
			}
		}
	 ...
	 */
	private void onFailure(TransportMessage message) {
		if (message.getTransportListener() != null) {
			try {
				message.getTransportListener().onFailure();
			} catch (Exception e) {
				// badly defined listener shouldn't crash the thread
				// so catch all exceptions here and do nothing
				e.printStackTrace();
