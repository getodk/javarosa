...
/**
 * The definition of a Question to be presented to users when filling out a
 * form.
 * 
 * QuestionDef requires that any IDataReferences that are used are contained in
 * the FormDefRMS's PrototypeFactoryDeprecated in order to be properly
 * deserialized. If they aren't, an exception will be thrown at the time of
 * deserialization.
 * 
...
public class QuestionDef implements IFormElement, Localizable {
	...
	// int dataType -- can we get away with storing this only in the instance
	// node?
	...
	private String shortText; /*
							 * The prompt text. The text the user sees in short
							 * modes.
							 */
	...
	// this may be used in the future, but it is not the default value you're
	...
	public boolean equals(Object o) {
		if (o instanceof QuestionDef) {
			...
			return (id == q.id
					...
					&& ExtUtil.equals(ExtUtil.nullIfEmpty(selectItemIDs),
							ExtUtil.nullIfEmpty(q.selectItemIDs)) && ExtUtil
					.equals(ExtUtil.nullIfEmpty(selectItemsLocalizable),
							ExtUtil.nullIfEmpty(q.selectItemsLocalizable)));
	...
	// this function is dangerous: QuestionDef will not serialize properly
	// unless selectItemIDs is set as well
	...
	public void setSelectItemIDs(OrderedHashtable selectItemIDs,
			Vector selectItemsLocalizable, Localizer localizer) {
	...
	// calling when localizer == null is meant for when there is no localization
	// data and selectIDMap contains only
	...
	public void localizeSelectMap(Localizer localizer) {
		...
		for (int i = 0; i < selectItemIDs.size(); i++) {
			...
			boolean translate = ((Boolean) selectItemsLocalizable.elementAt(i))
					.booleanValue();
			if (translate) {
				label = (localizer == null ? "[itext:" + i + "]" : localizer
	...
	/*
	 * public IAnswerData getDefaultValue() { return defaultValue; }
	 * 
	 * public void setDefaultValue(IAnswerData defaultValue) { this.defaultValue
	 * = defaultValue; }
	 */
	...
	public void readExternal(DataInputStream dis, PrototypeFactory pf)
		...
		setTitle((String) ExtUtil.read(dis, new ExtWrapNullable(String.class),
				pf));
		setAppearanceAttr((String) ExtUtil.read(dis, new ExtWrapNullable(
				String.class), pf));
		setLongText((String) ExtUtil.read(dis,
				new ExtWrapNullable(String.class), pf));
		setShortText((String) ExtUtil.read(dis, new ExtWrapNullable(
				String.class), pf));
		setHelpText((String) ExtUtil.read(dis,
				new ExtWrapNullable(String.class), pf));
		setLongTextID((String) ExtUtil.read(dis, new ExtWrapNullable(
				String.class), pf), null);
		setShortTextID((String) ExtUtil.read(dis, new ExtWrapNullable(
				String.class), pf), null);
		setHelpTextID((String) ExtUtil.read(dis, new ExtWrapNullable(
				String.class), pf), null);

		setControlType(ExtUtil.readInt(dis));

		setSelectItemIDs((OrderedHashtable) ExtUtil
				.nullIfEmpty((OrderedHashtable) ExtUtil.read(dis,
						new ExtWrapMap(String.class, String.class, true), pf)),
				ExtUtil.nullIfEmpty((Vector) ExtUtil.read(dis, new ExtWrapList(
						Boolean.class), pf)), null);
		if (getSelectItemIDs() != null
				&& (controlType == Constants.CONTROL_SELECT_MULTI || controlType == Constants.CONTROL_SELECT_ONE)) {
			localizeSelectMap(null); // even for non-multilingual forms, text
										// must be initially 'localized'
		}

		binding = (IDataReference) ExtUtil.read(dis, new ExtWrapNullable(
	 ...
	 * 
	 * @see
	 * org.javarosa.core.util.Externalizable#writeExternal(java.io.DataOutputStream
	 * )
	 */
	public void writeExternal(DataOutputStream dos) throws IOException {
		...
		ExtUtil.write(dos, new ExtWrapMap(ExtUtil
				.emptyIfNull(getSelectItemIDs())));
		ExtUtil.write(dos, new ExtWrapList(ExtUtil
				.emptyIfNull(selectItemsLocalizable)));

		ExtUtil.write(dos, new ExtWrapNullable(binding == null ? null
				: new ExtWrapTagged(binding)));
	}
